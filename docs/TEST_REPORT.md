...
### The cause, found and fixed at its source; second run, one full hour

The cause named above was only half right. The lever is not a smoothing across carriers and the trigger is
not an echo: it smooths the pilot grid over TIME (+-12 symbols), after fitting every symbol onto the
frame-mean of the raw pilots. A residual carrier offset turns each symbol by a common phase - about 10 degrees
a symbol was measured as the tuner drifted, some 350 degrees over the 35-symbol frame - and the mean of a
phasor that has gone once round is nearly nothing. Odd and even symbols carry their pilots on different
carriers, so the two halves of that mean collapse differently, and the data cells of alternate symbols came
out scaled x0.77 and x1.30. Raw symbol power was flat and the per-symbol estimate was flat; only the smoothed
one alternated. That is why it grew slowly, why re-acquiring (a fresh frequency estimate) "cured" it, and why
it looked like a settling tuner at start-up. The reference receiver now de-rotates each symbol before the
mean (its margin gate gained a phase-ramp leg whose negative control is the old build: 0/74 at a full turn).

Second same-samples run, **the full 60 minutes**, the reference on its CPU path too so the fixed code ran
live on both sides:

| 60 min, identical samples | this GNU Radio chain (live) | reference receiver (live, CPU path) |
|---|---|---|
| frames decoded | 14557 | 14560 |
| FEC blocks BCH-clean | 1077217 / 1077218 | 1077440 / 1077440 |
| re-acquisitions | **0** | **0** |
| sample-continuity breaks | 0 | 0 |
| whole frames shed, decoder behind | 2 (start-up) | - |
| retries by the healthy-SNR safety net | 0 needed | - |
| IP datagrams | 1269178 | 1269483 |
| datagrams byte-identical in both | **1269178 = every one of ours**; the reference's other 305 are this chain's start-up and the last second in flight | |

SNR over the hour: median 20.3 dB, minimum 17.8. No loss event on either side.

**Control - did the bug condition even occur in that hour?** A clean hour proves nothing if it did not. The same
hour of samples through the reference receiver's OLD build (its `ATSC3_CE_DEROT=0` switch, which exists for
exactly this): **966155 / 1052798 blocks (91.8 %), 139 re-acquisitions**, some 86,600 blocks lost between
minutes 3 and 12 - about five minutes of television. The fixed build, same samples: none, and none. The safety-net retry stays in
the Frame Decoder - it costs nothing until a frame fails at a healthy SNR - but it was not needed once.
