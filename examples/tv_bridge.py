...
IP datagrams on purpose, and this example is the join.
  ...
  tv_bridge.py --soapy "driver=..." --freq HZ [--antenna NAME] [--gain ELEMENT=VALUE ...]
               --live-dir DIR [--seconds N] [--play] [--audio]

--play   starts the reference receiver's tools/atsc3_play.py on DIR (video, a few seconds behind).
--audio  also starts tools/atsc3_audio.py (AC-4 -> PCM) so its full viewer can be pointed at DIR.
...
def main():
    ...
    with site_lock("atsc3rx-tv") as lk:
        ...
        def start_helpers():
            # the reference players read the lane list ONCE, at launch: start them when there is a video lane
            if a.audio:
                helpers.append(subprocess.Popen([sys.executable, "-u", os.path.join(tools, "atsc3_audio.py"),
                                                 "--live-dir", live_dir], cwd=a.receiver,
                                                stdout=open(os.path.join(live_dir, "audio.log"), "ab"),
                                                stderr=subprocess.STDOUT))
            if a.play:
                cmd = [sys.executable, "-u", os.path.join(tools, "atsc3_play.py"), "--live-dir", live_dir,
                       "--lag", str(a.lag), "--player", a.player]
                if a.player_args:
                    cmd += ["--player-args", a.player_args]
                helpers.append(subprocess.Popen(cmd, cwd=a.receiver,
                                                stdout=open(os.path.join(live_dir, "play.log"), "ab"),
                                                stderr=subprocess.STDOUT))
        ...
        while not stop.is_set():
            ...
            if int(time.time() - t0) % 5 == 0:
                print(f"[{time.time() - t0:6.1f}s] frames {dec.n_frames}  FEC {dec.n_bch}/{dec.n_fec} BCH-clean  "
                      f"re-acq {sync.n_reacquire}  dropped {sync.n_dropped}  media segments {tv.n_seg}  "
                      f"live {tv.live.bytes / 1e6:.1f} MB", flush=True)
    ...
    for p in helpers:                        # the players tail a file: let them drain, then ask them to go
        try:
            p.wait(timeout=max(5.0, a.lag + 5.0))
