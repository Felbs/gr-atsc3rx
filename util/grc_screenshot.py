...
(it IS GRC's own canvas and its own File > Screen Capture code), with no window.
  ...
  python grc_screenshot.py examples/rx_capture.grc docs/img/grc_rx_capture.png

GRC_BLOCKS_PATH must include this repo's grc/ directory so the atsc3rx blocks resolve."""
import sys
...
def main(grc_file, png):
    ...
    app = Gtk.Application(application_id="org.gnuradio.atsc3rx.screenshot")
    app.register(None)
