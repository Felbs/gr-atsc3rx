...
window, and stops it cleanly (the radio is closed by the flowgraph, never by killing it).
  ...
  python run_qt_shot.py build/grc/rx_radio_qt.py --set freq=<Hz> --set antenna="<port>" \
...
def main():
    ...
    with site_lock("atsc3rx-grc") as lk:
    ...
    if p is not None:
        pct = 100.0 * p.clean / p.fec if p.fec else 0.0
        print(f"ran {time.time() - t0:.0f} s: {p.frames} frames, {p.clean}/{p.fec} FEC blocks BCH-clean ({pct:.2f}%)")
        return 0 if p.clean else 1
    return 0
